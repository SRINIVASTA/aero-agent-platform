...
class AeroOrchestrator:
    def __init__(self, api_key: str):
        # We allow initialization with an empty string so the app doesn't crash on boot if offline
    ...
    def route_message(self, session_id: str, user_message: str) -> str:
        ...
        if self.client and self.api_key != "OFFLINE":
            try:
                ...
                # Gemini failed or credits ran out! Silently drop into the rule engine below
                pass

        # --- PATHWAY B: The Gemini-Free Local Rule Backup Engine (If Credits Run Out) ---
